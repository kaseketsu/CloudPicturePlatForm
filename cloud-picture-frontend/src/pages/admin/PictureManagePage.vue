    ...
    >
      <template #bodyCell="{ column, record }">
        ...
        <template v-else-if="column.key === 'action'">
          <a-space wrap>
            <a-button
              v-if="record.reviewStatus !== PIC_REVIEW_STATUS_ENUM.ACCEPT"
              type="link"
              @click="handleReview(record, PIC_REVIEW_STATUS_ENUM.ACCEPT)"
              >通过
            </a-button>
            <a-button
              danger
...
//定义数据
const dataList = ref<API.Pictures[]>([])
const total = ref(0)
...
const handleReview = async (record: API.Pictures, reviewStatus: number) => {
  const reviewMessage =
    reviewStatus === PIC_REVIEW_STATUS_ENUM.ACCEPT ? '管理员审核通过' : '管理员审核拒绝'
